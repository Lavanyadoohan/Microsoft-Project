    ...
    def main():
        ...
        while(True):
            ...
            for rect in rects:
                ...
                mask, end_points_left = eye_tracker.eye_on_mask(mask, left, shape)
                mask, end_points_right = eye_tracker.eye_on_mask(mask, right, shape)
                ...
                thresh = eye_tracker.process_thresh(thresh)
                
                eyeball_pos_left = eye_tracker.contouring(thresh[:, 0:mid], mid, img, end_points_left)
                eyeball_pos_right = eye_tracker.contouring(thresh[:, mid:], mid, img, end_points_right, True)
                eye_tracker.print_eye_pos(img, eyeball_pos_left, eyeball_pos_right)
